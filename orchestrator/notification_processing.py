...
from config.email_config import (
    EMAIL_THREAD, EMAIL_BODY_SECTION_TEMPLATE, 
    EMAIL_BODY_TEMPLATE, EMAIL_TABLE_CELL_TEMPLATE, 
    EMAIL_TABLE_HEADER_TEMPLATE, EMAIL_TABLE_ROW_TEMPLATE,
    EMAIL_BODY_SUMMARY_TEMPLATE, SUMMARY_TABLE_ROW_TEMPLATE)
...
class NotificationHandler:
    ...
    def __init__(self, run_id: str, postgres_connector,table_names: dict, smtp_config=SMTP_CONFIG, email_thread=EMAIL_THREAD,output_dir = "./test/test_output"):
        ...
        try:
            ...
            for row in rows:
                self.results.append({
                    ...
                    'payload_snapshot': json.loads(row[6]) if row[6] and isinstance(row[6], str) else row[6],
                    'created_at': row[7],
                    'success_entities': json.loads(row[8]) if row[8] and isinstance(row[8], str) else row[8],
                    'failed_entities': json.loads(row[9]) if row[9] and isinstance(row[9], str) else row[9],
                    'skipped_entities': json.loads(row[10]) if row[10] and isinstance(row[10], str) else row[10],
    ...
    def _fetch_entities_from_results(self,) -> dict:
    ...
    def send_notification(self, body: str, attachment_paths: list = None):
        ...
        try:
            ...
            try:
                with open(f'logs/pdm2ec_log_{datetime.now().strftime("%Y-%m-%d_%H")}.log', 'rb') as f:
                    part = MIMEBase('application', 'octet-stream')
                    part.set_payload(f.read())
                    encoders.encode_base64(part)
                    part.add_header('Content-Disposition', f'attachment; filename="pdm2ec_log_{datetime.now().strftime("%Y-%m-%d_%H")}.log"')
                    msg.attach(part)
                    logger.info(f"Attached log file: pdm2ec_log_{datetime.now().strftime('%Y-%m-%d_%H')}.log")
    ...
    def send_pipeline_notification(self):
        ...
        try:
            ...
            logger.info(f"Pipeline notification prepared and sent for run {self.run_id}")
        except Exception as e:
    ...
    def _save_payloads_to_file(self) -> str:
        ...
        payload_data = {
            ...
            "total_warnings": len([f for f in self.results if f['status'] == 'WARNING']),
            "total_successes": len([f for f in self.results if f['status'] == 'SUCCESS']),
            "results": []
        }
    ...
    def _prepare_email_body(self):
        ...
        if not self.run_summary:
            return "<html><body><h2>Error: Pipeline run not found in database</h2></body></html>", []
        ...
        if len(self.results) > 100:  # Threshold for "too large"
            ...
            content = "\\n".join(sections)
            return EMAIL_BODY_TEMPLATE.format(content_sections=content), attachment_files
    ...
    def _build_overview_section(self) -> str:
        ...
        created_with_warnings = len({
            f['user_id'] for f in self.results
            if f['operation'] == 'CREATE' and f['status'] == 'WARNING'
        })
        created_without_warnings = max(0, (self.run_summary['created_count'] or 0) - created_with_warnings)
        ...
        metrics = [
            ...
            ("Status", f"<span style='color: {'green' if self.run_summary['status'] == 'SUCCESS' else 'red'};'><b>{self.run_summary['status']}</b></span>"),
            ("Started At", self.run_summary['started_at'].strftime("%Y-%m-%d %H:%M:%S") if self.run_summary['started_at'] else "N/A"),
            ("Finished At", self.run_summary['finished_at'].strftime("%Y-%m-%d %H:%M:%S") if self.run_summary['finished_at'] else "N/A"),
            ("Duration", duration),
            ("Total Records Processed", self.run_summary['total_records']),
            ("━━━ New Employees ━━━", "<b></b>"),
            ("  • Created (Success)", f"<span style='color: green;'><b>{created_without_warnings}</b></span>"),
            ("  • Created (With Warnings)", f"<span style='color: orange;'><b>{created_with_warnings}</b></span>" if created_with_warnings > 0 else "0"),
            ...
            ("  • Updated Successfully", f"<span style='color: green;'><b>{self.run_summary['updated_count']- len({f['user_id'] for f in self.results if f['operation'] == 'UPDATE' and f['status'] == 'WARNING'})}</b></span>"),
            ("  • Updated with Warnings", f"<span style='color: orange;'><b>{len({f['user_id'] for f in self.results if f['operation'] == 'UPDATE' and f['status'] == 'WARNING'})}</b></span>" if len({f['user_id'] for f in self.results if f['operation'] == 'UPDATE' and f['status'] == 'WARNING'}) > 0 else "0"),
            ("━━━ Terminations ━━━", "<b></b>"),
            ("  • Terminated", self.run_summary['terminated_count']),
            ("━━━ Issues ━━━", "<b></b>"),
            ("  • Failed", f"<span style='color: red;'><b>{self.run_summary['failed_count']}</b></span>" if self.run_summary['failed_count'] > 0 else "0"),
            ("  • Total Warnings", f"<span style='color: orange;'><b>{self.run_summary['warning_count']}</b></span>" if self.run_summary['warning_count'] > 0 else "0"),
        ]
    ...
    def _build_batch_section(self) -> str:
        ...
        headers = "".join([
            EMAIL_TABLE_HEADER_TEMPLATE.format(header=h) 
            for h in ["Batch Context", "Total Users", "Users with Changes", "Status", "Duration"]
        ])
        ...
        for batch in self.batches:
            ...
            cells = [
                EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<b>{batch['batch_context'] or 'Unknown'}</b>"),
                EMAIL_TABLE_CELL_TEMPLATE.format(cell=batch['total_users']),
                EMAIL_TABLE_CELL_TEMPLATE.format(cell=batch['users_with_changes']),
                EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<span style='color: green;'>{batch['status']}</span>" if batch['status'] == 'COMPLETED' else batch['status']),
    ...
    def _build_field_changes_section(self) -> str:
        ...
        for batch_context, changes in changes_by_batch.items():
            headers = "".join([
                EMAIL_TABLE_HEADER_TEMPLATE.format(header=h) 
                for h in ["User ID", "Field Name", "Old Value (EC)", "New Value (PDM)"]
            ])
            ...
            for change in changes:
                ...
                cells = [
                    EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<b>{change['user_id']}</b>"),
                    EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<code>{change['field_name']}</code>"),
                    EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<span style='color: #999;'>{ec_val}</span>"),
                    EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<span style='color: #0055A4;'><b>{pdm_val}</b></span>"),
                ]
                rows.append(EMAIL_TABLE_ROW_TEMPLATE.format(row_cells="".join(cells)))
    ...
    def _build_failures_section(self) -> str:
        ...
        if errors:
            sections_html.append(self._build_failure_table(errors, "❌ Failures", is_error=True))
        
        if warnings:
            sections_html.append(self._build_failure_table(warnings, "⚠️ Warnings", is_error=False))
    ...
    def _build_failure_table(self, items, title, is_error=True) -> str:
        ...
        for item in items:
            ...
            cells = [
                ...
                EMAIL_TABLE_CELL_TEMPLATE.format(cell=f"<span style='color: {'red' if is_error else 'orange'}; word-wrap: break-word; white-space: pre-wrap;'>{display_message}</span>"),
            ]
            rows.append(EMAIL_TABLE_ROW_TEMPLATE.format(row_cells="".join(cells)))
    ...
    def _build_summary_email(self) -> str:
        ...
        if self.results and any(f.get('payload_snapshot') for f in self.results):
            notes.append("<li><strong>payloads_{run_id}.json</strong> - All payload snapshots for debugging</li>".replace("{run_id}", self.run_id))
        
        notes.append("<li><strong>detailed_report_{run_id}.html</strong> - Complete pipeline report with all sections (batches, field changes, failures/warnings)</li>".replace("{run_id}", self.run_id))
        ...
        def failed_unique_count(op_name: str) -> int:
            return len({f['user_id'] for f in self.results if f['operation'] == op_name and f['status'] == 'FAILED'})
        
        def warning_unique_count(op_name: str) -> int:
            return len({f['user_id'] for f in self.results if f['operation'] == op_name and f['status'] == 'WARNING'})
        ...
        template = EMAIL_BODY_SUMMARY_TEMPLATE.replace(
            "<h3>Summary:</h3>",
            f"{attachments_note}<h3>Summary:</h3>"
        )
